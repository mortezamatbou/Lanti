...
//            $this->where = !empty($temp_where) ? $temp_where . ' ' . $this->where : $this->where;
    ...
    private function replace_get_build() {
        foreach ($this->getStatments as $statement => $holder) {
            ...
            if ($holder == '_') {
                ...
                $prefix = $this->get_statement_dependency($statement);
    ...
    private function get_statement_dependency($dependency) {
    ...
    private function insert_build() {
        $sql = "INSERT INTO :from: :value:";
        $this->query_string = $sql;
        $this->replace_insert_build();
    }

    private function replace_insert_build() {
    ...
    private function update_build() {
        $sql = "UPDATE :from: SET :value: WHERE :where:";
        $this->query_string = $sql;
        $this->replace_update_build();
    }

    private function replace_update_build() {
    ...
    private function delete_build() {
        $sql = "DELETE FROM :from: WHERE :where:";
        $this->query_string = $sql;
        $this->replace_delete_build();
    }

    private function replace_delete_build() {
